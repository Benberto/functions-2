...
// CODE HERE
const postTaxPrices = prices.map(tax => tax * .07 + tax );
...
// CODE HERE
const totalPopulation = populations.reduce( (total, amount) => total + amount);
...
const ordersTotal = orders.map(function(num){
...
const bobsTotal = purchases.filter(function(bob){
  ...
  return bob.owner === "Bob"}).reduce(function(acc, elem){
